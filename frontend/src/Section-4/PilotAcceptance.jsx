...
import { FaCheckCircle, FaUserShield, FaUsers, FaPlane } from "react-icons/fa";
      ...
      <div className="space-y-5">
        ...
        {selectedTrades.includes("AE") && aeData.fuelQty && (
          ...
          >
            ...
            <p className="text-sm text-green-600 dark:text-green-400 mt-3 flex items-center gap-2">
              ...
              {assignedPersonnel["AE"]?.[0]?.user.rank?.rank_of === "S"
                ? `${assignedPersonnel["AE"]?.[0]?.user.user_name} ${assignedPersonnel["AE"]?.[0]?.user.rank?.abbreviation}`
                : assignedPersonnel["AE"]?.[0]?.user.rank?.rank_of === "O"
                  ? `${assignedPersonnel["AE"]?.[0]?.user.rank?.abbreviation} ${assignedPersonnel["AE"]?.[0]?.user.user_name}`
                  : `${assignedPersonnel["AE"]?.[0]?.user.user_name}`}{" "}
              ({assignedPersonnel["AE"]?.[0]?.user.pno})
            </p>
          </motion.div>
        )}
        ...
        >
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
            Pilot PIN: <strong>{PILOT_PIN}</strong>
          </p>
