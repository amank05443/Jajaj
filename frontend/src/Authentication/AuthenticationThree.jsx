...
export default function AuthRightSide({ snowId, authRight }) {
  const { params, loading } = useParams();
  const [open, setOpen] = useState(false);
  const [supTrade, setSupTrade] = useState(null);
  ...
  //  -------------------------------- INITIAL SUPERVISOR AUTHENTICATION -----------------------------------------------
  const handleSupervisorAuth = async () => {
    try {
      ...
      const res = await fetch("/api/checkPasskey/", {
        ...
        body: JSON.stringify({
          byWhom: supervisor,
          passkey: supervisorPasskey,
        }),
      });
      const data = await res.json();
      if (data.status == "OK") {
        const res = await fetch("/api/e-sign-entry/", {
          method: "POST",
          headers: {
            "X-CSRFToken": csrfToken,
            "Content-Type": "application/json",
        ...
        alert("First user Authenticated & saved!");
        setInitialAuthDone(true);
      } else {
        console.log(data);
        alert("Invalid supervisor or Passkey ");
      }
    } catch (err) {
